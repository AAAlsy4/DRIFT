"""
Batch SIFT + RANSAC UAV--satellite registration.

Outputs:
    batch_sift_results.csv
    batch_sift_summary.txt
...
def sift_ransac_one_pair(
    ...
    kp1, des1 = sift.detectAndCompute(img_uav, None)
    kp2, des2 = sift.detectAndCompute(img_sat, None)
    ...
    # 按描述子距离排序，保留前 max_matches 个，类似控制 RANSAC 输入规模
...
def save_results(output_dir: str, results: List[Dict[str, float]]) -> None:
    ...
    csv_path = output_dir / "batch_sift_results.csv"
    summary_path = output_dir / "batch_sift_summary.txt"
...
def parse_args():
    parser = ArgumentParser(description="Batch SIFT + RANSAC for UAV-satellite registration.")
