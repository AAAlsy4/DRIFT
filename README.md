...
- [1. 项目简介](#1-项目简介)
- [2. 方法流程图与可视化结果](#2-方法流程图与可视化结果)
- [3. 环境配置](#3-环境配置)
- [4. 数据组织方式](#4-数据组织方式)
- [5. 运行方法](#5-运行方法) 
  - [5.1 单对图像配准](#51-单对图像配准)
  - [5.2 批量运行 RoMa + Top-K + RANSAC](#52-批量运行-roma--top-k--ransac)
  - [5.3 对比实验](#53-对比实验)
- [6. 评价指标](#6-评价指标)
- [7. 实验结果](#7-实验结果)
  - [7.1 特征提取方法参数对比](#71-特征提取方法参数对比)
  - [7.2 对比实验](#72-对比实验)
  - [7.3 Top-K 匹配点筛选消融实验（ransac_thr = 5.0）](#73-top-k-匹配点筛选消融实验ransac_thr--50)
  - [7.4 RANSAC 阈值消融实验（num_match = 2000）](#74-ransac-阈值消融实验num_match--2000)
  - [7.5 模块消融实验](#75-模块消融实验)
- [8. 实验结论](#8-实验结论)
- [9. 项目结构](#9-项目结构)
- [10. 创新点](#10-创新点)
...
1. 输入无人机图像和卫星图像；
2. 使用 RoMa 进行稠密特征匹配，得到候选匹配点及置信度；[README.md](../OCGNet/README.md)
...
3. 构建了针对无人机—卫星图像配准的实验评价体系，从匹配点数量、内点数量、内点率、重投影误差及可视化结果等多个维度，对Top-K参数和RANSAC阈值进行了系统分析，为跨视角遥感图像配准参数选择提供了实验依据。
